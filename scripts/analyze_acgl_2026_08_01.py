...
def build_inputs() -> AnalysisInputs:
    return AnalysisInputs(
        ...
        # 사실 확인 - 준비금 적정성(호의적 발전 지속, 신용등급 오히려
        # 상향)·자본배분(BVPS +22.6%, 자사주매입을 M&A보다 우선)·거버넌스
        # (창업자 지배구조 없음, 내부자매도 미미) 전부 양호. 다만 재보험
        # 부문 ex-cat 컴바인드레이쇼가 2분기 연속 소폭 악화(+130~160bp YoY)
        # 중이고, 2025년 LA 산불 등 대재해 리스크는 DCF의 평활화된 FCF로는
        # 전혀 반영되지 않는다는 구조적 한계가 재확인됨.
        falsification_conditions=(
            "(1) ex-cat 컴바인드레이쇼가 3개 분기 연속 악화되거나 200bp"
            "YoY를 초과해 벌어지면(현재 '가격규율 유지' 서사의 근거가 "
            "무너짐) 이 판정은 재검토 대상. (2) 대형 대재해 손실(허리케인 "
            "등)로 특정 분기 cat load가 두 자릿수 %p를 넘으면 평활화된 "
            "FCF 가정 자체가 깨진 것으로 간주. (3) S&P/AM Best가 등급을 "
            "하향하거나 전망을 negative로 바꾸면 준비금 적정성 재평가 필요."
        ),
