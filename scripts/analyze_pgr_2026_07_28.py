...
def build_inputs() -> AnalysisInputs:
    return AnalysisInputs(
        ...
        # 사실 확인 - 텔레매틱스 데이터우위(Snapshot)가 실제 손해율 우위로는
        # 이어지지 않고 있음(2025년 컴바인드레이쇼 GEICO~84/Allstate~85.0 <
        # PGR~88 - PGR이 점유율은 1위이나 손해율은 4위 중 3위). 신규계약 증가율도
        # 11%(연초)→8%YoY(2026-04)로 둔화 중. 상업용차 라인에서 소폭 역발전
        # 준비금($140M, H1 2025) - 업계 전체의 다년간 문제(사회적 인플레이션)와
        # 같은 계열. SBC/희석은 PGR 사상 최초로 이 트래커에서 교차검증했고
        # 순이익의 ~2.4%로 무해함(다만 정확한 SEC 원자료 기준 sbc_by_year는
        # 아직 엔진에 배선 안 함 - WebSearch 추정치라 정밀 대조 보류).
        falsification_conditions=(
            "(1) 신규계약(PIF) 증가율이 2026년 내내 한 자릿수 초반까지 "
            "추가 둔화되거나, GEICO/Allstate 대비 손해율 열위가 확대되면 "
            "(현재 '점유율은 1위, 손해율은 3위' 구도가 더 악화됨을 뜻함) "
            "텔레매틱스 모트 서사를 재검토. (2) 상업용차 역발전 준비금이 "
            "$140M보다 유의미하게 확대되거나 개인용차 라인까지 번지면 "
            "재검토 대상. (3) 2026-08-04 Q2 실적(이 조사 하루 뒤 예정)에서 "
            "컴바인드레이쇼가 90 이상으로 뛰면(수리비 인플레이션 재점화 "
            "신호) 재검토."
        ),
