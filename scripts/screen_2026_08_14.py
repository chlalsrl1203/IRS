...
한도 도달로 2종목은 재무데이터 확보 보류.** UPS는 Amazon 물량 50%+ 감축
확정으로 매출 자체가 회사 가이던스상 역성장 예정 - 마진개선 서사는 있으나
이 FCF-DCF 프레임이 답할 질문이 아니라 판단, 확인 전 제외. CVS·UNH·CNC는
셋 다 헬스케어보험업의 구조적 압박(의료급여율 급등·상환율 정체·정책
보조금 폐지)으로 개별종목 미스프라이싱이 아닌 섹터 리스크라 판단, 확인
전 제외. AFRM·SOFI는 BNPL/디지털뱅크 특유의 신용손실충당금 변동성+
상업화 초기 FCF 불안정 가능성으로 ARDX·EOSE와 유사한 프레임워크 부적합
소지 - 확인 전 제외. 주택건설업(Lennar·D.R. Horton 등)은 금리환경에
업종 전체가 동시에 눌리는 매크로 현상이라 개별 미스프라이싱 탐지라는
이 스크리닝의 목적과 안 맞아 확인 전 제외.
...
**Alpha Vantage 일일 API 한도(25회) 도달 + FMP도 플랜 제한**으로 이번
세션에서 재무데이터 확보를 시도하지 못해 PASSED_INITIAL_SCREEN에
서사만 기록해두고 다음 세션(API 한도 리셋 후)으로 넘겼다 -
PASSED_INITIAL_SCREEN 누적 6건(SAP·DDOG·PLTR·IONS·PYPL·BABA).
"""
...
# ── 후속 조사 12차(동일 08-14 세션, 사용자 "계속" 요청) ──
PREFILTERED_OUT["UPS(United Parcel Service)"] = (
    "Amazon 물량을 2026-06까지 50% 이상 감축하는 합의로 3분기 미국 일평균 "
    "물량이 중한자릿수%대 역성장할 것으로 회사가 직접 가이던스 제시"
    "(WebSearch, 2026-08-15) - 저마진 아마존 물량을 고마진 헬스케어·SMB "
    "고객으로 대체하는 전략적 마진개선 서사는 있으나, **회사 스스로 매출 "
    "감소를 확정적으로 예고한 상태**라 이 스크리너의 MIN_REALISTIC_GROWTH"
    "(8%) 임계값을 구조적으로 넘기 어렵다고 판단 - '마진개선이 저평가를"
    "만드는지'는 이 FCF-DCF 프레임이 답하기 어려운 질문이라 확인 전 단계"
    "에서 제외."
)
PREFILTERED_OUT["CVS(CVS Health)/UNH(UnitedHealth)/CNC(Centene)"] = (
    "셋 다 헬스케어보험업의 구조적 압박 - CVS는 Aetna 부문 의료급여율 "
    "급등(고령층 의료이용률이 보험계리 추정 상회), UNH는 상환율 정체(CMS "
    "2026년 인상률 0.09%)와 의료비 급증 간 격차 확대로 수십년 만에 첫 "
    "매출감소 경고, CNC는 Medicare Part D 보조금($36억) 2027년 폐지 "
    "우려(WebSearch, 2026-08-15) - 셋 다 서사가 아니라 정책·손해율이라는 "
    "실측 가능한 요인에서 나온 4분류 3번(진짜나빠짐)에 가깝다. 업종 전체가 "
    "동시에 눌리는 구조적 문제라 개별종목 미스프라이싱보다는 섹터 리스크에 "
    "가깝다고 판단, 재무데이터 확보 전 제외."
)
PREFILTERED_OUT["AFRM(Affirm)/SOFI(SoFi Technologies)"] = (
    "각각 -35%/-38%YTD, 소비자신용 사이클 우려·AI가 대출심사를 대체할 "
    "것이라는 서사(WebSearch, 2026-08-15) - BNPL(선구매후결제)·디지털뱅크 "
    "특유의 신용손실충당금 변동성이 크고 상업화 초기 단계 특성상 FCF가 "
    "불안정할 가능성이 높아 ARDX·EOSE와 유사한 프레임워크 부적합 소지, "
    "확인 전 단계에서 제외."
)

# ── 주택건설업 - 개별종목 미스프라이싱이 아니라 업종 전체 구조적 냉각 ──
PREFILTERED_OUT["주택건설업(Lennar/D.R. Horton 등)"] = (
    "10년물 국채금리 4.5%대 고착으로 30년 모기지 금리 ~6.5%, 신용평가사가 "
    "2026년 단독주택 착공 전망을 '증가'에서 '-4.5% 역성장'으로 하향, 주요 "
    "건설사들이 인센티브 확대(12.9%)·평균판매가 인하로 대응 중(WebSearch, "
    "2026-08-15) - 개별종목이 시장에 오분류된 게 아니라 업종 전체가 금리 "
    "환경에 동시에 눌리는 매크로 현상이라 이 스크리닝(개별 미스프라이싱 "
    "탐지)의 목적과 안 맞는다고 판단, 확인 전 단계에서 제외."
)
...
PASSED_INITIAL_SCREEN = {
    ...
    "PYPL(PayPal)": (
        "5년 고점 대비 -83%, 2026년 -22%YTD(WebSearch, 2026-08-15) - "
        "'AI가 결제·대출 심사 모델을 붕괴시킨다'는 서사가 NOW·ACN와 동일한 "
        "AI붕괴 공포 계열이다. 여전히 대형 결제플랫폼으로 실제 매출·이익이 "
        "있어(단일제품 바이오텍류 프레임워크 부적합과는 무관) 확인 가치가 "
        "높다고 판단했으나, **Alpha Vantage 일일 API 한도(25회) 도달 + "
        "FMP도 플랜 제한으로 이번 세션에서 재무데이터 확보를 시도하지 "
        "못했다** - 다음 세션(API 한도 리셋 후)에서 우선 확인할 후보로 "
        "남겨둔다."
    ),
    "BABA(Alibaba)": (
        "2026년 -22%YTD, 중국 소비둔화·PDD/JD와의 경쟁심화·규제우려"
        "(WebSearch, 2026-08-15) - 일부 분석은 'China ADR 리스크 프리미엄이 "
        "장기평균 대비 과도하게 넓어 할인이 과할 수 있다'고 평가. **주의**: "
        "이 프로젝트가 이미 PDD 스크리닝에서 확인한 '규제·경쟁 이슈가 "
        "뚜렷한 업종(플랫폼·중국기업·반독점 노출)은 스크리닝 Gap을 그대로 "
        "신뢰하지 말 것'이라는 알려진 한계가 정확히 적용되는 사례 - "
        "competition_intensity 상수 placeholder 문제가 PDD와 동일하게 "
        "작동할 가능성이 높다. 재무데이터 확보 시 정성적 경쟁강도를 반드시 "
        "함께 볼 것(스크리닝 Gap만으로 판단 금지)."
    ),
}
