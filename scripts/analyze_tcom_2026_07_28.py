...
def build_inputs() -> AnalysisInputs:
    return AnalysisInputs(
        ...
        # ADS(나스닥 상장분) 실제 거래통화는 USD - 재무제표 통화(RMB)와 별개로
        # "그때 얼마였는지"는 실제 거래가격 기준(PDD/SE 등 기존 관행과 동일 원칙).
        price_at_analysis=44.77,
        currency="USD",

        # 2026-08-04 정성심층조사(A등급 6종목 배치) 반증조건 - 결과 미확정
        # 미래시점 이벤트 기준. 근거: SEC CORRESP로 확인된 中규제리스크 서술
        # 완화 이력(현 증권소송 알레고리와 직결) + 회사 자체 Q2 2026 가이던스
        # (+3~8%)가 모델의 Realistic Growth(12.39%)를 크게 하회 - MNDY 선례와
        # 유사한 자체가이던스 하향 패턴.
        falsification_conditions=(
            "1) Q2/Q3 2026 실제 매출성장률이 가이던스(+3~8%YoY) 수준 또는 "
            "그 이하로 확정되면, Realistic Growth(12.39%)가 회사 자체 전망과 "
            "괴리된 낙관적 입력이었음이 확인되므로 재실행 필요. 2) De Wilde "
            "증권소송(E.D.N.Y., 집단기간 2024-04-30~2026-01-13)이 화해 또는 "
            "판결로 종결되고 그 내용이 SEC 공시완화 관행에 대한 실질 책임을 "
            "인정하면 거버넌스 리스크 재평가. 3) 중국-일본 여행 코리더 급감 "
            "(최대 -50% 전망)이 실제로 현실화되며 대체 코리더(한국/베트남/"
            "태국)로의 전환이 매출믹스/마진에 유의미한 악영향을 미치면 재검토."
        ),
        ...
        data_sources=[
            ...
            "2026-08-04 정성심층조사(A등급): 자본배분·회계품질(SEC CORRESP 공시완화 이력 "
            "발견)·거버넌스(증권소송 진행중)·中규제리스크·희석·DRS 73.6(트래커 최고) 타당성 "
            "재확인 - 판정불변, 자체가이던스(+3~8%)가 Realistic Growth(12.39%) 하회 확인, "
            "Confidence 재검토 권고(94→미검증 유지)",
        ],
    )
