...
def build_inputs() -> AnalysisInputs:
    return AnalysisInputs(
        ...
        # v3.24+(2026-08-02, S등급 4종목 중 3종목 심층조사 - PDD/MNDY/DUOL):
        # 엔진이 못 보는 부분(자본배분·회계품질·거버넌스·희석)을 WebSearch로
        # 별도 조사한 결과, SBC/희석은 문제없음(FCF의 6~8%, 트래커 하위권)으로
        # 확인됐으나 2026-01 SAMR(중국 시장감독총국)과의 물리적 충돌·임원 체포·
        # 확대된 사기·탈세 조사(2026-04 RMB15.1억 벌금 포함)가 새로 발견됐다 -
        # DRS의 competition_intensity(이미 20점 만점)와는 성격이 다른 거버넌스·
        # 컴플라이언스 리스크로, 기존 판정을 뒤집을 근거는 아니었으나
        # Confidence 89를 그대로 신뢰하기엔 부담이 있다고 판단(체감상 70%대가
        # 더 적절 - 엔진 필드로 표현할 방법은 없어 이 주석과 Notion에만 기록).
        falsification_conditions=(
            "(1) 차기 실적(2026-08-31 Q2 예정)에서 매출성장률이 재차 5% 미만으로 "
            "둔화되거나 순이익 감소폭이 -15%보다 더 벌어지면(현재 마진압박이 "
            "일시적 재투자가 아니라 구조적 경쟁열위로 전환된 신호) 이 판정은 "
            "틀린 것으로 간주. (2) 2026-08-28 EU DSA 컴플라이언스 시한 이후 "
            "추가 제재나 Temu 유럽사업 축소가 발생하거나, SAMR 조사가 임원 "
            "형사처벌·RMB 10억 단위를 초과하는 벌금으로 확대되면 거버넌스 "
            "리스크가 이미 반영된 정성적 추정치를 넘어선 것으로 보고 재검토."
        ),
