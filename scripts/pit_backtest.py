...
#
# ⚠️ **디스크 용량 제약(2026-08-29 실측)**: companyfacts는 종목당 약 3~4MB라
# 300종목에 1.2GB, 1,200종목에 약 4.8GB다. 전체 유니버스(약 8,900종목)를
# 캐시하면 **약 35GB**로, 이 실행 환경의 가용 디스크(약 17GB)를 넘긴다.
# 전체 유니버스로 PIT를 돌리려면 셋 중 하나가 필요하다:
#   (a) 유니버스를 샤딩해 샤드마다 캐시를 비우며 진행
#   (b) 캐시를 원본 JSON이 아니라 **추출된 (지표, 회계연도, filed, 값) 튜플**로
#       저장 - truncate에 필요한 건 그것뿐이라 용량이 수백 배 줄어든다
#   (c) 캐시 없이 T0 하나만 돌리기(다중 T0 검증을 포기하므로 비권장)
# (b)가 옳은 방향이지만, 실증된 필요(전체 유니버스 PIT 요구)가 생겼을 때
# 구체적 요구사항을 갖고 만든다 - Simplicity First.
