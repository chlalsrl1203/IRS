...
def main():
    ...
    print("\n" + "=" * 108)
    print("⚠️ 8차 - IT컨설팅/서비스 'AI가 개발자를 대체한다' 공포 테마(2026-07-31)")
    print("   EPAM/ACN/CTSH 전부 screen() 탈락 - 대형 IT서비스는 매출기반 자체가")
    print("   커서 구조적으로 8% 성장 문턱을 못 넘는다(공포과잉 여부와 무관)")
    print("=" * 108)
    for tickers, reason in {
        "EPAM(EPAM Systems)": (
            "S&P500 낙폭 5위(YTD-48%). screen() 탈락 - 현실적성장률 6.63%<8.0%"
            "(제약 FCF CAGR 7.38%). 2022 러시아-우크라이나 전쟁으로 동유럽 "
            "엔지니어 인력 기반 사업이 타격받아 FY2023 매출 -2.78% 역성장한 "
            "여파가 5년 CAGR을 끌어내림. 2026 가이던스도 유기적성장 3~6%로 "
            "스스로 감속 예고 + 최대고객(NEORIS) 이탈 + AI코딩툴에 의한 "
            "커미디티화 우려 겹침 - 공포과잉이라 보기 어려움."
        ),
        "ACN(Accenture)": (
            "screen() 탈락 - 현실적성장률 6.01%<8.0%(제약 FCF CAGR 6.68%). "
            "시총 $101.5B 초대형주라 매출 자체가 이미 $70B 규모 - 구조적으로 "
            "고성장이 불가능한 성숙 스톨워트형 비즈니스."
        ),
        "CTSH(Cognizant)": (
            "매출 5년 CAGR 3.34%로 문턱(8.9%)에 크게 못 미쳐 FCF 확인 없이도 "
            "탈락 확정 - ACN과 동일하게 대형 성숙 IT서비스업 구조적 한계."
        ),
    }.items():
        print(f"  {tickers}")
        print(f"    -> {reason}")
