...
def build_inputs() -> AnalysisInputs:
    return AnalysisInputs(
        ...
        # 미래시점 이벤트 기준(사후합리화 아님). 근거: ROIC 9.15%(5y평균
        # 20.74%대비 급락, 단일출처 미검증)+TBS세그먼트 마진 30% vs Cyber
        # Safety 61%+시장은 27%매출성장에도 -9%(멀티플압축)로 반응+대형
        # 매도데스크 2곳 중립유지. 판정/Gap/RAR 불변, 병기만.
        falsification_conditions=(
            "1) FY2027 H2(2027-08~2027-10경 실적발표) '핵심사업 외 파트너 "
            "확장으로 연 $100M 증분매출' 목표 미달성시 크로스셀 서사 재검토. "
            "2) ROIC가 9.15%대에서 정체/추가하락하고 5y평균(20.74%) 대비 "
            "회복 기미 없으면 MoneyLion 인수가 가치창출이 아닌 가치희석으로 "
            "확정 - 재검토. 3) Trust-Based Solutions 세그먼트 마진이 30%에서 "
            "추가 악화되거나 Cyber Safety 마진(61%)이 5%p 이상 하락하면 "
            "재검토. ⚠️ ROIC 9.15%는 단일출처(ainvest.com)로 10-K 대조 "
            "미실시 - 다음 재분석 시 1차 검증 필요."
        ),
        ...
        data_sources=[
            ...
            "2026-08-04 정성심층조사(A등급): M&A자본배분·ROIC·세그먼트마진·거버넌스·희석·"
            "경쟁동향 - 판정불변, Confidence 재검토 권고(89→미검증 상태 유지, 다음 세션 판단)",
        ],
    )
