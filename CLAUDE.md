...
## v3.88~v3.89 — 반증조건 자동 준비 + 실전 트랙레코드 주기 측정
(2026-09-18, 사용자 "현실적으로 가장 우리의 기능을 올리는 아이디어" → 실행)

이 프로젝트의 CLAUDE.md 전체에 같은 문장이 반복된다 — **"실현결과와의 관계
증거가 0건이다."** growth_quality(v3.53)·accounting_quality(v3.70)·
dilution(v3.84)이 전부 계산은 되지만 판정에 배선되지 않은 이유가 이 하나이고,
Confidence가 `UNCALIBRATED`인 이유도, H-001~H-006이 전부 BLOCKED인 이유도 같다.
새 진단 모듈을 하나 더 만드는 것보다 **그 공백 자체를 메우기 시작하는 것**이
기능을 올린다고 판단해 두 가지를 배선했다.

### ① engine/thesis_checkpoints.py (v3.88) — check_by가 와도 아무도 안 알려줬다

`engine/thesis.py`(v3.48)는 `invalidation_conditions`에 `check_by`를 남길 수
있지만 그 날짜가 와도 통보 경로가 없었다 — v3.42가 이미 겪은 실패(반증조건
트리거 5건이 전부 지났는데 12일간 아무도 안 열어봄)가 thesis 계층에서 반복될
구조였다. 이제 기한 도래(due)·임박(approaching)을 찾아 **연결된 ledger 대비
SEC에 새 연차 데이터가 있는지**까지 붙여 보여준다.

⚠️ **판정하지 않는다.** v3.42의 "정규식은 트리거 날짜와 서술적 날짜를 구분 못
한다"가 여기서는 "구조화된 조건 문구도 자동 판정 대상이 아니다"로 이어진다 —
`mark_invalidation_triggered()`는 여전히 사람이 호출한다. '기한이 됐는가'는
날짜 산술이고 '조건이 맞았는가'는 판단이라는 구분이 이 모듈의 존재 이유다
(테스트로 고정 — `decide`/`judge`류 공개 함수가 생기면 실패).

### ② engine/track_record.py (v3.89) — 합성이 아닌 진짜 T0→결과를 쌓는다

v3.73~v3.78 PIT 백테스트는 **합성**(과거로 되돌아가 재구성)이었다. 이건 v3.24
`price_at_analysis`가 채워지기 시작한 뒤부터 **우리가 그날 실제로 내린 판정**과
이후 실제 주가를 대조한다. 한 번 재고 끝나면 의미가 없어서(같은 날 1차 측정이
일회성 스크립트였다) `broad_screen.yml`에 주 1회 자동 실행으로 배선하고,
`reports/track_record/`에 스냅샷을 쌓는다.

**⚠️ 이 측정이 즉시 실제 결함 두 개를 잡았다 — 그게 이 모듈의 값어치다.**

**(a) 진입가 시점 규칙이 하루씩 틀려 있었다.** 첫 실행에서 57건 중 **34건**이
"ledger 기록가 ≠ 그날 종가"로 걸렸다. 원인은 데이터 오류가 아니라 **분석이
미국 장 개장 전(한국 오전)에 돌았다**는 것이었다 — 그 시각에 쓸 수 있는 가격은
그날 종가가 아니라 직전 거래일 종가다:

```
ADBE  analyzed_at 09-04 09:24 UTC(동부 05:24, 개장 전) -> ledger 285.75 = 09-03 종가
VRT   analyzed_at 09-04 23:02 UTC(동부 19:02, 마감 후) -> ledger 280.53 = 09-04 종가
```

`entry_bar_as_known_at()`으로 "그 시각에 실제로 알 수 있었던 종가"를 고르도록
고치니 불일치가 **34건 → 6건**으로 떨어졌다. DST는 `zoneinfo`(stdlib)가
처리한다 — UTC 상수 하나로 고정하면 여름/겨울에 하루씩 어긋난다(테스트로 고정).

**(b) 남은 6건은 ledger가 묵은 시세를 쓴 것이었다.** `trace_ledger_price()`가
어느 거래일 종가였는지 역추적한다: DUOL·NBIX **2거래일**, SKYW·UBER·WDAY
**1거래일**, **TCOM 5거래일** 묵은 시세. 즉 그 6종목의 Gap은 분석 시점보다
1~5거래일 낡은 시가총액으로 계산됐다. 과거 ledger를 소급 수정하지 않는다
(v3.32 원칙) — 대신 매 측정마다 이 사실이 리포트에 드러난다.

**수익률 정의도 바로잡았다** — 진입·청산을 **같은 배당조정 시계열**에서 뽑는다.
ledger 원종가로 나누고 현재 조정종가를 쓰면 그 사이 배당만큼 조용히 틀린다
(1차 일회성 스크립트가 그랬다).

**1차 측정(57종목, 보유 6~48일·중앙값 14일)**: 전체 중앙값 **−2.30%**(양수
29.8%). 저평가 판정군(n=34) −1.78% vs 적정가군(n=23) −2.58%로 **방향은 기대와
일치하나 격차가 작다**. 등급별로는 단조 순서가 안 나온다(S −1.43%, A −1.93%,
B −4.81%, C −2.58%). 이 창(2026-08~09) 자체가 소프트웨어 섹터 전반 매도세였고
n·기간 모두 결론을 낼 수준이 아니다 — `MEASUREMENT_STATUS =
OBSERVATIONAL_NOT_INFERENTIAL`로 리포트가 매번 스스로 그 사실을 말한다.

⚠️ **사전등록 실험이 아니다.** `experiments/`의 H-001~H-006을 대체하지 않고,
그 exit_rule(H-006은 최소 15건)을 우회하는 근거로도 쓰지 않는다. 측정값은
`reports/`에만 남고 **Gap·등급·Confidence·사이징 어디에도 되먹임되지 않는다**
(테스트로 고정 — `weight`/`verdict`류 공개 함수가 생기면 실패).

**폐기**: 같은 날 만든 일회성 `scripts/live_track_record_2026_09_18.py`와 그
리포트를 `git rm`했다 — 진입 시점 규칙과 배당조정이 둘 다 틀린 판본이라
남겨두면 같은 날짜에 서로 다른 숫자를 가진 파일이 둘 생긴다(v3.32가 실측한
중복 오염 사고와 같은 형태).

**검증**: 테스트 1,193 → **1,225개 전부 통과** · baseline fingerprint
`f5709edf…` **불변** · **`ledger/`·공식 판정 0건 수정** · `ENGINE_VERSION`
v3.87 → **v3.89**.

⚠️ **자동화가 실제로 돌려면 main 병합이 필요하다** — scheduled 워크플로는
기본 브랜치의 `.yml`만 읽는다(2026-09-18 오전에 실측 확인한 그 문제).

## thesis 체크포인트를 일일 감시에 배선 (2026-09-18, 사용자 "적용")

v3.88이 만든 `engine/thesis_checkpoints.py`가 **수동 CLI로만 존재**했다.
`check_by` 기한이 와도 사람이 기억해서 돌려야 확인되는 상태 — v3.42가 이미
겪은 실패(반증조건 트리거 5건이 전부 지났는데 12일간 아무도 안 열어봄)가
thesis 계층에서 그대로 재발할 구조였다.

### 배선 지점을 브리핑이 아니라 `run_monitor()`로 잡았다

`daily_brief.py`에 직접 넣으면 **표시만** 되고 이슈 제목 긴급도에는 반영되지
않는다(브리핑은 `urgency_key="routine"`으로만 올린다). 반면 `run_monitor()`에
넣으면 한 지점에서 셋이 동시에 된다 — 브리핑 표시(브리핑이 이 함수를 부른다) ·
감시 단독 경로 표시 · `action_required`를 통한 제목 긴급도 상승.
브리핑이 이미 감시를 재구현하지 않고 위임하는 구조라(자기 docstring에 명시)
같은 패턴을 그대로 따른 것이기도 하다.

이제 감시는 세 축이다: ledger 반증조건 · 동결 예측 해소기한 · **thesis
`check_by` 기한**.

### ⚠️ `enrich_with_sec_freshness()`는 쓰지 않는다

`due_conditions()`(순수 날짜 산술)만 쓴다. 후자는 SEC 조회가 필요한데, 이
감시와 브리핑의 핵심 성질이 **"네트워크 의존이 전혀 없어 스크리닝이 통째로
죽어도 정상 작동한다"**(v3.64)는 것이다. 그 하나를 위해 전체를 외부 API에
묶으면 이 감시가 막으려는 실패가 그대로 재발한다. SEC 신선도 확인은 사람이
의도적으로 돌리는 `scripts/thesis_checkpoint_prep.py`에 그대로 남겨둔다.

**판정하지 않는 경계도 그대로다** — 브리핑 본문이 "기한이 됐다는 뜻이지 조건이
맞았다는 뜻이 아니다"를 명시하고 `mark_invalidation_triggered()`로 되돌린다.
"반증 확정"류 어휘가 본문에 들어가면 테스트가 실패한다.

### ⚠️ 작성 중 스스로 잡은 것 2건

**① 「총 N건」이라 쓸 뻔했다.** `due_conditions()`는 도래·임박·날짜없음 세
갈래만 돌려주고 **경고창(30일) 밖의 미래 기한은 어느 갈래에도 없다.** 세
갈래를 더해 총계라고 부르면 실제보다 적은 수가 총계로 찍힌다(실측: 저장소
thesis 조건 **23건**인데 세 갈래 합은 **19건** — NOW 2027-01-31·PTC×2/VRT
2026-11-30 4건이 창 밖이라 빠진다). 총계 주장을 걷어내고 회귀 테스트로 고정.

**② 문자열 금지 테스트가 자기 설명을 위반으로 잡았다.** `enrich_with_sec_
freshness`를 소스에서 단순 검색으로 막았더니 **왜 안 쓰는지 설명하는 주석**이
걸렸다 — 그대로 두면 판단 근거를 코드에서 지우는 쪽으로 압력이 생긴다.
실제 호출·import만 잡도록 AST 검사로 바꿨고, 위반 3형태(직접호출·속성접근·
import)가 전부 잡히고 주석만 있는 경우는 통과함을 역검증했다.

### 실측(2026-09-18)

thesis 조건 23건 중 기한 도래 **0건**(가장 가까운 것이 2026-11-30로 73일 뒤),
날짜없는 상시감시 19건. 즉 오늘은 조용하고, 그게 정상 동작이다 — 대신
**「기한 0건이 '볼 게 없다'는 뜻은 아니다」**를 매번 함께 표시한다(19건이
날짜 없이 상시 감시 대상이므로). 이 프로젝트가 반복 경계해온 "데이터 없음을
안전으로 오독"의 같은 형태다.
...
## v3.91 — 대시보드 자가갱신 전환: 페이지가 스스로 원자료를 읽는다
(2026-09-19, 사용자 요청 "이 html 계속 볼 수 있게 해줘. 매일 새롭게 갱신되는건
당연하고")

### 왜 v3.90으로는 "매일 갱신"이 성립하지 않았나

v3.90 대시보드는 파이썬 생성기가 SVG·표를 렌더링해 HTML에 **박아넣은**
정적 페이지였다. 그래서 숫자를 바꾸려면 사람이 세션을 열어 페이지를 다시
발행해야 했고(= 매일 갱신 불가), 원자료 갱신도 **주간** 워크플로
(`broad_screen.yml`)에만 있었다. 이 스냅샷의 입력은 재무제표가 아니라
**매일 바뀌는 주가**라 주 1회면 최대 6일 묵은 값이 뜬다 - 주간 스크리닝이
매일 돌 이유가 없는 것과 정확히 반대 이유다.

### 실측으로 확인한 것 하나가 설계를 통째로 바꿨다

저장소가 **공개**이고 `raw.githubusercontent.com`이
`access-control-allow-origin: *`를 보낸다(2026-09-19 직접 확인). 따라서
**브라우저가 저장소 JSON을 직접 읽을 수 있다** - 세션도, 예약 실행도,
재발행도 필요 없다. 처음엔 매일 세션을 깨워 재발행하는 Routine을 걸려고
했는데, 그 경로는 세션 가용성에 의존하고 실패해도 조용하다.

- `dashboard/portfolio_track_record.html`(신설) - 열릴 때마다
  `reports/portfolio_track_record/latest.json`을 직접 가져와 렌더링한다.
- `daily_screen.yml`에 `scripts.portfolio_track_record_ci` 단계 추가
  (`if: always()`, 시세 출처만 쓰므로 Finviz·SEC·AV가 죽어도 독립 실행).
  커밋 대상에 `reports/portfolio_track_record/` 추가 - 러너는 매번 새로
  뜨므로 커밋하지 않으면 사라진다.
- `LATEST_NAME = "latest.json"`(engine 상수) - 날짜 박힌 파일만 있으면
  페이지가 날짜를 **추측**해야 하고, 주말·휴일·CI 실패로 그날 파일이
  없으면 조용히 빈 화면이 된다. 상수를 엔진에 둔 이유는 v3.35가 실측한
  것과 같다(같은 문자열을 스크립트·테스트·HTML에 각자 박으면 한쪽만
  고쳤을 때 조용히 어긋난다).

### ⭐ 렌더러를 한 벌만 둔다 - 파이썬 생성기를 저장소로 승격하지 않았다

정적 생성기를 `scripts/`로 옮기는 안을 먼저 검토했다가 기각했다. 옮기면
렌더링 로직이 파이썬과 자바스크립트 **양쪽**에 생기고(중복 계산이 두 계산을
미묘하게 어긋나게 만든다 - Simplicity First), 그러고도 재발행 없이는
숫자가 안 바뀐다. 지금 렌더러는 HTML 안 하나뿐이고 파이썬은 JSON만
만든다. 스크래치패드의 생성기·템플릿은 저장소에 들이지 않았다.

### ⚠️ 조회 실패 시 낡은 숫자를 보여주지 않는다

빌드 시점 스냅샷을 내장해두면 네트워크가 죽었을 때 낡은 값이 최신인 척
표시된다 - 이 프로젝트가 반복 경계해온 "데이터 없음을 안전으로 오독"
(is_insurer·sbc_cross_check·holdings_overlap)의 화면판이다. 대신 실패
사실과 시도한 URL을 그대로 띄우고 본문을 숨긴다. 스냅샷이 4일 넘게
묵으면(평일만 도는 워크플로라 금→월 3일이 정상) "자동 갱신이 멈췄을 수
있다"는 경고를 따로 낸다.

### 조용히 깨지는 경로 둘을 테스트로 고정했다

**스키마 드리프트가 가장 위험하다** - 파이썬 스냅샷의 키 이름이 바뀌어도
자바스크립트는 `undefined`를 읽고 **예외 없이 빈칸을 그린다.** R-001에서
`fcf0` 키 오타 하나로 사전등록 6축 중 1축이 조용히 죽어 있던 사고의 웹
버전이다. `snap.<key>`·`r.<key>` 참조를 HTML에서 추출해 **실제 스냅샷에
그 키가 있는지** 검사한다(`tests/test_portfolio_dashboard.py`, 11건).
고정 경로 이름이 세 곳에서 갈리는 경우, 빌드 치환자가 되살아나는 경우,
스냅샷이 내장되는 경우, `latest.json`이 최신 dated 스냅샷과 갈리는 경우도
함께 고정했다.

### ⚠️ 기존 테스트 하나를 상태 단언에서 불변조건으로 다시 썼다

`test_ci_script_writes_exactly_once_and_only_into_its_own_out_dir`가 "쓰기
호출이 **정확히 1곳**"을 요구해 `latest.json`이라는 정당한 두 번째 쓰기에
실패했다. 개수는 지켜야 할 성질이 아니라 그 시점의 상태였다 - 진짜
불변조건("모든 쓰기 경로가 `--out-dir`에서 조립되는가")을 **쓰기 호출
하나하나에** 적용하도록 고쳤다(BRO `model_choice_reason`·
`test_every_prediction_starts_open`과 같은 처리). 약해진 게 아니라
새로 생기는 쓰기를 전부 검사하므로 오히려 강해졌다.

### 검증

Chromium으로 실제 렌더링을 확인했다(로컬 HTTP로 같은 JSON을 서빙) - 18행·
추이선 2개·수치 전부 정상, 390px 폭에서 가로 스크롤 없음, JS 예외 0건.
404 상황도 직접 재현해 본문이 숨겨지고 실패 배너가 뜨는 것을 확인했다.

테스트 1,252 → **1,263개 전부 통과** · baseline fingerprint `f5709edf…`
**불변** · `ledger/`·공식 매수리스트·판정 **0건 수정** ·
`ENGINE_VERSION` v3.90 → **v3.91**(engine 상수 추가 - v3.32 규칙).

### ⚠️ 남은 한 가지 - main에 병합돼야 실제로 뜬다

페이지는 `main` 브랜치의 `latest.json`을 읽는데, 이 변경은 아직 작업
브랜치에만 있다(main에는 `reports/portfolio_track_record/` 자체가 없다).
병합 전까지 페이지는 위 실패 배너를 정직하게 띄운다 - 병합되는 순간부터
평일 매일 자동 갱신된다.
