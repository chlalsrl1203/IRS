...
**⚠️ 미해결 항목 - 보험업 FCF-DCF 방법론 (2026-07-28 PGR에서 재확인, ACGL이
최초 발견)**: `pipeline.py`는 보험사의 OCF가 플로트(float, 보험료 선수취-
보험금 후지급) 성장으로 회계이익보다 구조적으로 부풀려진다는 점을 보정하는
로직이 없다. 성장기 보험사에 FCF-DCF를 그대로 적용하면 내재가치가 체계적으로
과대평가될 수 있다(ACGL v3.13 핵심노트: "Gap+31.44%p/RAR3.003은 FCF-DCF를
자본집약적 보험업에 적용한 데서 오는 과장일 가능성 높음"). 지금까지는 종목별로
ROE x 유보율(지속가능성장률)과 P/B를 수작업 교차검증하는 방식으로 우회했다
...
가능성이 있어 판정 신뢰도를 낮춰야 했다). **의도적으로 아직 배선하지
않았다** - capex_classification(v3.20) 때와 같은 이유로, insurer 표준
조정치(예: 배당+자사주매입을 진짜 FCF로 보는 관행, 또는 준비금 증가분을
OCF에서 차감하는 관행)에 대한 업계 컨센서스가 이 프로젝트에서 아직
충분히 실증되지 않았다. 다음에 보험업 종목을 분석할 때는 매번 손으로
교차검증을 반복하지 말고, 이 두 사례(ACGL/PGR)를 기반으로
`AnalysisInputs`에 `is_insurer: bool` 플래그를 추가해 자동으로
ROE x 유보율 교차검증을 계산·경고하는 기능을 pipeline.py에 배선할 것.
