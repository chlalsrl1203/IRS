...
⚠️ 이 표는 **규모 조건부 캡 적용 전** 상태다(이 변경을 촉발한 관측). 적용 후
수치는 §10 참조 — 1% 미만은 0%, 5% 미만은 26.23%가 됐다.
...
| **`LYNCH_TYPE_CAPS`를 규모 조건부로 변경** | **ADOPTED (2026-08-23 사용자 명시 승인, `thr=1.0%`)** | 종목의 RG가 즉시 바뀌고 → Gap → 판정 → 매수 비중까지 연쇄한다. **자본 재배분**이므로 v3.28(ROP override 승격) 선례대로 명시적 승인을 받고 실행했다. 실제 영향 범위는 §10 참조 — **최초 제안서에 적은 범위가 틀렸고 정정 후 재승인받았다** |
| 캡 값 자체(25%/12%)를 다른 숫자로 교체 | **REJECT** | stalwart 12%는 실증적으로 문제없음이 확인됐다. 문제는 값이 아니라 규모 무시이므로 값만 바꾸는 건 오진에 대한 처방이다 |

## 10. 실행된 변경 — 규모 조건부 캡 (v3.67, 2026-08-23 승인 후 적용)

현행 `LYNCH_TYPE_CAPS`의 단일 상한은 그대로 두고, 그 **위에** 규모 구간별
base rate가 `thr=1.0%` 이상인 성장률을 추가 상한으로 얹는 방식
(`size_conditioned_growth_cap()`). 두 상한 중 낮은 쪽이 최종 RG가 된다.

### ⚠️ 승인 요청 시 내가 보고한 영향 범위가 틀렸다

제안서 초판은 **"PDD 단 1종목만 바뀐다(매수 비중 9.07%)"**라고 적었다.
**틀렸다.** 실제 영향은 **PDD·PGR·SE 3종목, 매수 비중 합계 28.46%**이고
SE는 등급까지 바뀐다(S→A).

**원인**: Lynch 캡이 **이미 바인딩된** 종목(DUOL·MNDY·PDD·BRO·CDNS·GEN·ROP)만
조사했는데, 규모 상한은 **Lynch 캡에 걸리지 않던 종목도 새로 잡는다** — PGR
(16.87%, >$50B)과 SE(23.56%, $12-25B)가 정확히 그 경우다. "기존 캡 바인딩
종목 = 영향 대상"이라는 잘못된 등식을 검증 없이 썼다. 사용자에게 이 오류를
공개하고 정정된 범위(3종목·28.46%)로 재확인받은 뒤 실행했다.

### 실측 영향 (전수)

| 종목 | 규모 구간 | RG 변경 전 | RG 변경 후 | Gap 변경 전 | Gap 변경 후 | 등급 |
|---|---|---|---|---|---|---|
| **PDD** | >$25B | 25.00% | **17.87%** | +29.16%p | **+22.04%p** | S 유지 |
| **PGR** | >$50B | 16.87% | **12.75%** | +21.76%p | **+17.64%p** | S 유지 |
| **SE** | $12-25B | 23.56% | **17.87%** | +19.18%p | **+13.50%p** | **S → A** |
| DUOL·MNDY | $700M-1.25B | 25.00% | 변화 없음 | — | — | 유지 |
| BRO·CDNS·GEN | $3-4.5B | 12.00% | 변화 없음 | — | — | 유지 |
| ROP | $4.5-7B | 5.50%(override) | 변화 없음 | — | — | 유지 |

세 종목 모두 판정은 "저평가 가능성"으로 유지되며 **매수 유니버스 진입·이탈은
0건**이다(SE는 S→A로 등급만 이동, 둘 다 유니버스 안).

### 포트폴리오 노출 변화

| base rate 등급 | 변경 전 | 변경 후 |
|---|---|---|
| EXTREMELY_RARE (<1%) | **16.46%** (PDD·SE) | **0.00%** |
| RARE (<5%) 누적 | 41.02% | **26.23%** (PGR 12.00 + DUOL 7.13 + MNDY 7.10) |

**1% 미만 base rate 위에 서 있던 자본 16.46%가 0이 됐다.** 5% 미만은
26.23%가 남는데, 전부 규모 조건부 상한이 `thr=1.0%`를 만족하는 범위 안에
있다(PGR 3.9% / DUOL·MNDY 3.5%) — 컷을 1%로 잡았으므로 이는 설계대로다.

### 최종 매수 비중

GEN 18.00 · ACGL 12.00 · PGR 12.00 · TCOM 10.36 · **PDD 7.76** · DUOL 7.13 ·
MNDY 7.10 · BRO 6.19 · **SE 5.89** · UBER 5.40 · WDAY 5.11 · TTD 3.06
(합계 정확히 100%).

⚠️ `thr=1.0%`는 여전히 **검증된 컷이 아니다**(§11-1). 승인 시 사용자에게 이
사실을 명시했고 사용자가 그 값을 선택했다.
